...
const AppNav = memo(({ isReady }: { isReady: boolean }) => {
  ...
  return (
    <nav
      className={`flex flex-wrap gap-2 rounded-xl border border-slate-700 bg-slate-900 p-2 shadow-sm ${
        isReady ? "" : "opacity-70"
      }`}
    >
      {NAV_ITEMS.map((item) => (
...
const AppError = memo(({ message }: { message: string }) => (
  <section className="rounded-xl border border-red-700 bg-red-950 p-6 text-sm text-red-200 shadow-sm">
...
const AppLoading = memo(
  ({ message, progress }: { message: string; progress: number }) => (
    <section className="rounded-xl border border-slate-700 bg-slate-950 p-8 shadow-sm">
...
const App = memo(() => {
  ...
  useEffect(() => {
    // Force dark mode styling by enabling Tailwind "dark" class mode.
    document.documentElement.classList.add("dark");
  }, []);

  return (
    <DemoModulesProvider value={modulesSnapshot}>
      <main className="min-h-full bg-slate-950 text-slate-100 p-4 md:p-6">
